...
StudentModule::StudentModule(QWidget *parent)
  ...
  ui->setupUi(this);
  ui->openRuleBook_pushButton->setDisabled(false); // after implementation, make it true

  // using a button
  QPushButton *examTableActionBtn = new QPushButton("Enter");
  ui->exam_list_tableWidget->setCellWidget(0, 8, examTableActionBtn);

  connect(this, &StudentModule::fileReceived, this, &StudentModule::showMsgBox);
}
...
void _showMsgBox(QWidget *parent, QString type, QString title, QString text) { // currently useless
  if (type == "information") {
...
void StudentModule::showMsgBox(const QString& msg) {
    QMessageBox::information(this, "File Received", msg);
}
...
void StudentModule::showFileReceivedMsg(const QString& fileName) {
    QMessageBox::information(this, "File Received", fileName + " was received!");
}
