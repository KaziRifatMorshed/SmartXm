...
TeacherModule::TeacherModule(QWidget* parent) : QMainWindow(parent), ui(new Ui::TeacherModule) {
    ui->setupUi(this);
    ui->serverStatus_label_2->setText("<html><head/><body><p><span style=\" font-size:18pt;\">Server Status: "
                                      "NOT STARTED</span></p></body></html>");
    ui->serverIP_label_3->setText("<html><head/><body><p><span style=\" font-size:18pt;\">Server Local IP: "
                                  "NOT STARTED</span></p></body></html>");
    ui->label->setText("<html><head/><body><p align=\"center\"><span style=\" "
                       "font-size:20pt;\">Welcome, " +
                       QString::fromStdString(currentUser.getName()) + " Sir</span></p></body></html>");
    ...
    ui->connectedPCwithServer_tableWidget->horizontalHeader()->setStretchLastSection(false);
    ui->connectedPCwithServer_tableWidget->horizontalHeader()->setSectionResizeMode(0, QHeaderView::Stretch);
    ui->connectedPCwithServer_tableWidget->horizontalHeader()->setSectionResizeMode(1, QHeaderView::ResizeToContents);
    ui->connectedPCwithServer_tableWidget->horizontalHeader()->setSectionResizeMode(2, QHeaderView::ResizeToContents);
    ui->connectedPCwithServer_tableWidget->horizontalHeader()->setSectionResizeMode(3, QHeaderView::ResizeToContents);
    ui->connectedPCwithServer_tableWidget->setColumnCount(4);
    ui->connectedPCwithServer_tableWidget->setHorizontalHeaderLabels(QStringList() << "Name"
                                                                                   << "Stu ID"
                                                                                   << "Local IP"
                                                                                   << "Action");
    ...
    ui->allXmList_tableWidget->horizontalHeader()->setSectionResizeMode(0, QHeaderView::Stretch); // Course Name
    ui->allXmList_tableWidget->horizontalHeader()->setSectionResizeMode(1,
                                                                        QHeaderView::ResizeToContents); // Course Code
    ui->allXmList_tableWidget->horizontalHeader()->setSectionResizeMode(
        2, QHeaderView::ResizeToContents); // Course Teacher
    ui->allXmList_tableWidget->horizontalHeader()->setSectionResizeMode(3, QHeaderView::Stretch); // Exam Topic
    ui->allXmList_tableWidget->horizontalHeader()->setSectionResizeMode(4, QHeaderView::ResizeToContents); // Exam Date
    ui->allXmList_tableWidget->horizontalHeader()->setSectionResizeMode(5, QHeaderView::ResizeToContents); // Exam Time
    ui->allXmList_tableWidget->horizontalHeader()->setSectionResizeMode(6, QHeaderView::ResizeToContents); // Action
    ...
    connect(clientsUpdateTimer, &QTimer::timeout, this, &TeacherModule::showConnectedStudentInfo);
...
void TeacherModule::on_StartServer_toolButton_clicked() {
    ...
    if (server == nullptr) {
        ...
        ui->serverStatus_label_2->setText("<html><head/><body><p><span style=\" font-size:18pt;\">Server "
                                          "Status: " +
                                          QString::fromStdString(server->getStatus() + "</span></p></body></html>"));
        ui->serverIP_label_3->setText("<html><head/><body><p><span style=\" "
                                      "font-size:18pt;\">Server Local IP: " +
                                      QString::fromStdString(server->getLocalIP()) + "</span></p></body></html>");
    }
    else {
...
void TeacherModule::on_StopServer_toolButton_2_clicked() {
    ...
    if (server != nullptr && Server::isRunning()) {
        ...
        ui->serverIP_label_3->setText("<html><head/><body><p><span style=\" font-size:18pt;\">Server Local "
...
void TeacherModule::showConnectedStudentInfo() {
    ...
    for (int row = 0; row < connectedClientsNo; ++row) {
        ...
        ui->connectedPCwithServer_tableWidget->setItem(row, 0, new QTableWidgetItem(name));

        // Column 1: Stu ID
        ui->connectedPCwithServer_tableWidget->setItem(row, 1,
                                                       new QTableWidgetItem(QString::fromStdString(ci.studentID)));

        // Column 2: Local IP
        ui->connectedPCwithServer_tableWidget->setItem(row, 2, new QTableWidgetItem(QString::fromStdString(ci.ip)));
        ...
        connect(infoBtn, &QPushButton::clicked, this,
                [this, infoBtn]()
                {
                    ...
                    QMessageBox::information(this, "Connected Client",
                                             QString("Name: %1\nLocal IP: %2\nSocket FD: %3").arg(n).arg(ip).arg(fd));
                });

        // Create Button 2: Test Connection
        QPushButton* testClientConnectionBtn = new QPushButton("Test Connection");
        testClientConnectionBtn->setToolTip("Test Connection"); // Tooltip for clarity
        testClientConnectionBtn->setProperty("client_name", name);
        testClientConnectionBtn->setProperty("client_ip", QString::fromStdString(ci.ip));
        ...
        connect(
            ...
            {
                QString n = testClientConnectionBtn->property("client_name").toString();
                QString ip = testClientConnectionBtn->property("client_ip").toString();
                int fd = testClientConnectionBtn->property("client_socfd").toInt();
                bool t =
                    server->sendFileToClient(fd, "./resources/Hello2.pdf", "HELLO", "Hello packet sent from server");
                if (t) {
                    QMessageBox::information(
                        this, "Test Connection with Client",
                        QString("Name: %1\nLocal IP: %2\nSocket FD: %3\nHello file sent...").arg(n).arg(ip).arg(fd));
                }
                else {
                    QMessageBox::warning(this, "Test Failed",
                                         QString("Failed to send HELLO file to %1 (Socket: %2)").arg(n).arg(fd));
                }
            });
...
void TeacherModule::on_selectFile_pushButton_clicked() {
    ...
    instructionFileName = QFileDialog::getOpenFileName(this,
...
void TeacherModule::on_instruction_send_pushButton_clicked() {
    if (instructionFileName.length() <= 0) {
        ...
        FileMeta meta(title, fname, ext, std::time(nullptr), std::move(filedata), msg);
...
void TeacherModule::populateExamList() {
    ...
    if (!query.isActive()) {
        qWarning() << "Failed to execute exam list query:" << query.lastError().text();
        return;
    }
    ...
    while (query.next()) {
        ...
        // Column 4: Exam Date
        table->setItem(row, 4, new QTableWidgetItem(examDate.toString(Qt::ISODate)));
        ...
        connect(editBtn, &QPushButton::clicked, this,
                [this, examId]()
                {
                    ...
                    QMessageBox::information(this, "Edit Exam", QString("Would edit Exam ID: %1").arg(examId));
                });
        ...
        connect(deleteBtn, &QPushButton::clicked, this,
                [this, db, examId, row]()
                {
                    ...
                    reply = QMessageBox::warning(this, "Delete Exam",
                                                 QString("Are you sure you want to delete Exam ID %1?").arg(examId),
                                                 QMessageBox::Yes | QMessageBox::No);
                    if (reply == QMessageBox::Yes) {
                        QSqlQuery deleteQuery =
                            db->execQuery(QString("DELETE FROM Exam WHERE exam_id = %1").arg(examId));
                        if (deleteQuery.numRowsAffected() > 0) {
                            ui->allXmList_tableWidget->removeRow(row);
                        }
                        else {
                            QMessageBox::critical(this, "Error", "Failed to delete exam from database.");
                        }
                    }
                });
